...
load("C:/Users/Oli/Documents/PhD/wham_coupled/RCode/Tidy/WHAM_INFERNO_Data.RData")
...
B[,13] <- qunif(A[,13], min = 0.05, max = 0.1)
...
B[,19] <- qunif(A[,19], min = 0.5, max = 2) #Scaling factor for managed pasture
B[,20] <- qunif(A[,20], min = 0.5, max = 2) #Scaling factor for managed vegetation
...
for(z in 1:nrow(PFT_pars)) {
  ...
  for(i in 1:length(WHAM_ba)) {
    ...
    ### adjust managed by uncertainty pars
    Managed.adj <- (Managed_crop[[i]] + (Managed_cfp[[i]] * PFT_pars$Managed_veg[z]) + 
                    (Managed_defor[[i]] * PFT_pars$Managed_veg[z]) +
                    (Managed_pyrome[[i]] * PFT_pars$Managed_veg[z]) + (Managed_hg[[i]] * PFT_pars$Managed_veg[z]) + 
                      (Managed_pasture[[i]] * PFT_pars$Managed_pasture[z]))
    
    for(j in c(1:12)) {
      
      tot_WHAM[[i]][[j]]  <- WHAM_ba[[i]][[j]] + Managed.adj[[j]]
  ...
  ca <- brick(lapply(1:length(Managed), function(x) sum(comb.annual[[x]]) - Arable[[x]]))
  ...
  ### SSA uses Sentinel-2 @Jules resolution (Sentinel.JULES)
  ### Global uses GFED5 @Jules resolution (MOD.JULES)
